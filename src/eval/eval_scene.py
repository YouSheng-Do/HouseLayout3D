...
from metrics import f1_at_tau, pairwise_hausdorff, pairwise_rect, pairwise_stairs
...
def evaluate(scene: str, pred_dir: str, classes=("structures", "doors", "windows", "stairs")):
    """[eval_v2_nonrect — 2026-08-11] stairs 修正：載入**全部** GT 樓梯 mesh（含 7–14 頂點
    非矩形），distance matrix 依論文以 d_E（皆矩形）或廣義 d_H（任一非矩形，用 GT mesh 真面）
    混用。n_gt(stairs) 恆＝該場景 GT 樓梯總數（不再靜默丟）。舊版見 eval_v1_buggy.py。"""
    gt = load_scene_gt(scene, load_poses=False)
    ps, pd, pw, pst = load_pred_entities(pred_dir)
    gt_stairs = list(gt.stair_meshes)          # 全部 GT 樓梯 mesh（不過濾頂點數）
    out = {}
    pairs = {
        "structures": (gt.structure_entities, ps, pairwise_hausdorff),
        "doors": (gt.doors, pd, pairwise_rect),
        "windows": (gt.windows, pw, pairwise_rect),
        "stairs": (gt_stairs, pst, pairwise_stairs),
    }
    for cls in classes:
        g, p, fn = pairs[cls]
